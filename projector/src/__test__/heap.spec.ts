...
import { IReadNode, IWriteNode, getGlobalNode, ensureGlobalNode, INodeRef, isHeapRoot, createNode, IWriteNodeInternal, INodeRefInternal } from "../node";
import { globalState, IChangeMap } from "../mx/globalstate";
import * as heap from "../heap";
import { FieldDef, IType, TypeKind } from "../type";
import { heapSet, ensureHeap } from "../heap";
import { t0int32Type, t0boolType, t0doubleType, t0stringType, t0uaNodeRefType, t0childNodeType } from "../concept";
...
describe("heap", ()=> {
    ...
    it("heap and node works", ()=> {
        expect(heap.t0Heap.gidx).toBe(0);
        expect(heapSet.get(globalState.heapMap, heap.t0Heap.gidx)).toBe(heap.t0Heap);
        expect(getGlobalNode(heap.t0Heap.root.uid)).toBe(heap.t0Heap.root);
        expect(isHeapRoot(heap.t0Heap.root)).toBe(true);
        ...
        const nTypeEmpty = myHeap.getNodeType(nodeDef);
        expect(myHeap.getNodeType(nodeDef, [])).toBe(nTypeEmpty);
        const fields = [ 
        ...
        const nTypeWithFields = myHeap.getNodeType(nodeDef, fields);
        expect(myHeap.getNodeType(nodeDef, fields)).toBe(nTypeWithFields);
        ...
        expect(() => createNode(0, nTypeWithFields)).toThrowError(/Given content and recordType has different lengths/);
        expect(() => createNode(0, nTypeWithFields,[1])).toThrowError(/Given content and recordType has different lengths/);
        expect(() => createNode(0, nTypeWithFields,[1, true, undefined!, "ha", node])).toThrowError(/Undefined or atomValue field values are not allowed/);
        let baseNode: IReadNode = createNode(node.uid, nTypeWithFields, [1, true, 2.1, "ha", node ]);
        expect(() => createNode(node.uid, nTypeWithFields, [2, false, 3.1, "ba", node ])).toThrowError(/Record already exists/);
        ...
        // editable record
        const eChangeMap: IChangeMap = { isBase: false, isEditable: true };
        ...
        expect(() => node.write.setChild(fldSingleChildDef, childNode1, t0childNodeType)).toThrowError(/Unresolved editable node/);
        ...
        const childRecord1 = createNode(childNode1.uid, nTypeEmpty);
        ...
        expect(() => writeRecord.insertLast(fldMultiChildDef, childNode2, t0childNodeType)).toThrowError(/Unresolved editable node/);
        const childRecord2 = createNode(childNode2.uid, nTypeEmpty);
        const childRecord3 = createNode(childNode3.uid, nTypeEmpty);
        const childRecord4 = createNode(childNode4.uid, nTypeEmpty);
        expect(() => createNode(childNode4.uid, nTypeEmpty)).toThrowError(/Record already exists/);
        ...
        expect(() => childRecord3.insertBefore(childNode4)).toThrowError(/Cannot insert after a detached node or node from another heap/);
